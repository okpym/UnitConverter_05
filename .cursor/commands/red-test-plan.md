...
> PRD와 manifest Given/Then 충돌 시 **manifest** 우선. FR 원문 인용은 **PRD**에서만.
...
- **세션 주제** · **대상 Test ID** 는 현재 채팅 맥락과 `docs/PRD.md` FR에서 **자동 추출**한다.
...
| Rule | 내용 |
|------|------|
| **Rule1 — FR 인용** | PRD FR ID·원문 인용 (예: `FR-VAL-01` …) |
...
| Rule | 내용 |
|------|------|
| Rule1 | FR-VAL-01 — «value가 0 미만이면 변환을 수행하지 않고 거부한다» |
| Rule2 | meter:-1 입력 시 변환 결과가 stdout에 나오지 않음을 검증할 RED를 설계한다 |
| Rule3 | **VAL-01** — Given: meter:-1 / When: main() / Then: 음수 거부·변환 미출력 |
```
...
|----|------|
| Test ID | manifest ID |
| 대상 함수 | 프로덕션·테스트 진입점 (예: `main`, `_parse_unit_value`) |
| Given→Then | manifest Given에서 Then까지 한 줄 흐름 |
| Invariant | RED 중에도 **변하지 않아야 하는** 관찰 |
| Expected RED Failure | pytest FAIL 예상 (AssertionError·pytest.fail 등) |
...
| 함수명 | `test_<ID>_…` 네이밍 |
| conftest 픽스처 | `monkeypatch`, `capsys`, `manifest` 등 |
...
| Track | 점검 |
|-------|------|
| **Logic Track** | **Domain Mock 금지** — 변환 비율·단위 테이블 Mock 대체 금지 |
| **공통** | **E001~E005 emit 금지** — 테스트·플랜에 ECB 에러 코드 출력 시나리오 넣지 않음 |
| boundary / UI | stdin·stdout 관찰 위주; Harness(`monkeypatch`)만 |
...
| 금지 | 이유 |
|------|------|
| `src/` · `UnitConverter.py` · 프로덕션 코드 **수정** | Ask ③ — 설계만 |
...
**다음:** `/red-skeleton` — `pytest.fail` 스켈레톤 작성 (여전히 GREEN·프로덕션 수정 금지).
